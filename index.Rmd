...
- efficiency
- "efficiency plus one" [    ##    nts_attack_or_freeball (they got an attack or freeball attempt)
    ##    nts_eff_num (kill-error-block)
    ## NB nts_attack_or_freeball is redundant? because nts_eff_num will always be zero when this is FALSE?
    ##    but let's leave it in for now
    x %>% dplyr::filter(.data$skill %eq% "Attack") %>% group_by_at(grps) %>%
        dplyr::summarize(attack_plus1_efficiency = (sum(.data$evaluation %eq% "Winning attack")-sum(.data$evaluation %in% c("Error", "Blocked"))-
     sum(.data$nts_attack_or_freeball*.data$nts_eff_num))/n())
